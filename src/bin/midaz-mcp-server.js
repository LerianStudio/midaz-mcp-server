...
// Check for local Midaz services
async function checkLocalServices() {
  ...
  console.error('\n🔍 Checking for local Midaz services...');
...
function checkConfiguration() {
  ...
  console.error('💡 Run "midaz-mcp-config" to create a configuration file.\n');
  return null;
}
...
async function main() {
  ...
  process.on('SIGINT', () => {
    console.error('\n⏹️  Shutting down Midaz MCP Server...');
...
main().catch((error) => {
  console.error('❌ Failed to start Midaz MCP Server:', error.message);
  process.exit(1);
});